...
### DFS with Backtracking (Part 1):
```python
def count_all_paths(node, target, graph, visited):
    if node == target:
        return 1
    ...
    for neighbor in graph.get(node, []):
        if neighbor not in visited:
            count += count_all_paths(neighbor, target, graph, visited)
    
    visited.remove(node)
    return count
```

### DFS with Required Nodes (Part 2):
```python
def count_constrained_paths(node, target, graph, has_dac, has_fft, visited):
    if node == target:
    ...
    for neighbor in graph.get(node, []):
        if neighbor not in visited:
            count += count_constrained_paths(
                neighbor, target, graph, 
                new_has_dac, new_has_fft, visited
            )
...
def count_memo(node, has_dac, has_fft, visited_tuple):
    # Convert visited set to tuple for hashability
    # Cache based on current state
    ...
```
